...
const toGraph = (nodeArray) => {
    ...
    for (let i = 0; i < nodeArray.length; i += 1) {
        for (let j = 0; j < nodeArray[i].length; j += 1) {
            const nodeName = nodeArray[i][j].props.name;
            const neighbors = {};
            const top = i !== 0 ? nodeArray[i - 1][j].props.name : null;
            const bottom =
                i !== nodeArray.length - 1
                    ? nodeArray[i + 1][j].props.name
                    : null;
            const left = j !== 0 ? nodeArray[i][j - 1].props.name : null;
            const right =
                j !== nodeArray[i].length - 1
                    ? nodeArray[i][j + 1].props.name
                    : null;
            if (left) {
                neighbors[left] = 1;
            }
